...
public class ExplosiveBow extends MicroGame {
	...
	public String getAchievementForMsg() {
		...
		List<GamePlayer> achievement = getArena().getPlayers().stream().filter(x -> x.getAchievement() != null)
	...
	private void createColumns() {
		for (Square sq : getArena().getProperties().getSquares())
			sq.getLocations().forEach(location -> {
	...
	private void createWalls() {
		...
		Predicate<Location> predicate = isHard
				? l -> l.getBlockY() == first.getBlockY() + 3 || l.getBlockY() == first.getBlockY() + 2
				: l -> l.getBlockY() < first.getBlockY() + 3;
		if (isHard) {
			getArena().getProperties().destroySquares();
			locations.stream().filter(l -> l.getBlockY() == first.getBlockY())
					.forEach(l -> l.getBlock().setType(Material.AIR));
		}
		// FLOOR
		locations.stream().filter(predicate).forEach(l -> ManageHandler.getModernAPI()
		...
		locations.stream().filter(l -> l.getBlockY() >= first.getBlockY() + 3 && l.getBlockY() < first.getBlockY() + 9)
				...
				.forEach(l -> ManageHandler.getModernAPI().setBlock(
	...
	public void secondStartGame() {
		...
		ItemStack bow = ItemBuilder.start(requireNonNull(XMaterial.BOW.parseItem())).setUnbreakable(true)
				.addEnchantment(Enchantment.ARROW_INFINITE, 1, true).build();
	...
	public ItemStack getGameItemStack() {
		return ItemBuilder.start(requireNonNull(XMaterial.FIRE_CHARGE.parseItem()))
				.setDisplayName(ChatColor.RED + "&lEXPLOSIVE BOW FIGHT").build();
	}
	...
	public void event(Event event) {
		if (event instanceof EntityDamageEvent)
			...
			projectileHit(event);
		else if (!ManageHandler.getModernAPI().oldVersion() && event instanceof BlockExplodeEvent)
			blockExplode(event);
		else if (ManageHandler.getModernAPI().oldVersion() && event instanceof EntityExplodeEvent)
			entityExplode(event);
		else if (event instanceof EntityShootBowEvent)
			entityShootBow(event);
	}
	...
	private void entityDamage(Event event) {
		...
		if (!(isBlockExplosion || isEntityExplosion))
			return;
	...
	private void projectileHit(Event event) {
		...
		if (requireNonNull(getArena().getMicroGame()).getTime() < 5)
			return;
	...
	private List<Block> getDeleteBlocks(Location first, Location second, List<Block> blockList) {
		...
		for (Block block : blockList) {
			...
			if (itemBlock.isSimilar(XMaterial.GRASS_BLOCK.parseItem()))
				continue;
			...
			if (isRightFirstX || isRightSecondX || isRightFirstZ || isRightSecondZ)
				blocks.add(block);
		}
		return blocks;
	}
	...
	private void entityShootBow(Event event) {
		...
		if (requireNonNull(getArena().getPlayer(uuid)).getState() != State.PLAYING_GAME)
			return;
		GamePlayer gamePlayer = requireNonNull(getArena().getPlayer(uuid));
		if (requireNonNull(gamePlayer).getAchievement() == null)
			gamePlayer.setAchievement("1");
		else
			gamePlayer.setAchievement(Integer.toString(Integer.parseInt(gamePlayer.getAchievement()) + 1));
	}
