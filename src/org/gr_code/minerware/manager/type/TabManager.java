...
public class TabManager {
    ...
    public List<String> onTabComplete(CommandSender commandSender, Command command, String[] strings) {
        if (command.getName().equalsIgnoreCase("minerware")) {
            if (strings.length == 1)
                return filterList(commandSender.hasPermission("minerware.admin") ? subCommands : subCommandsNoneOp, strings);
            if (strings.length == 2) {
    ...
    protected ArrayList<String> subCommands = Lists.newArrayList("create", "edit", "setLobby",
           "spawnStatistic", "removeStatistic", "spawnLeaderboard", "removeLeaderboard", "forceStop", "forceStart", "version", "openGUI");

    protected ArrayList<String> subCommandsNoneOp = Lists.newArrayList("join", "randomJoin", "leave", "help");

    protected ArrayList<String> getSubArenas() {
        ArrayList<String> arenas = new ArrayList<>();
        MinerPlugin.getARENA_REGISTRY().iterator().forEachRemaining(arena -> arenas.add(arena.getProperties().getName()));
        return arenas;
    }
