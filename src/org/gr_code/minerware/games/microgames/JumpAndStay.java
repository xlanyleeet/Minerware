...
public class JumpAndStay extends MicroGame {
	...
	private void generateSlimes() {
		...
		cuboid.getLocations().stream().filter(l -> l.getBlockY() == f.getBlockY())
				.forEach(loc-> ManageHandler.getNMS().setBlock(requireNonNull(XMaterial.SLIME_BLOCK.parseItem()), loc.getBlock()));
		cuboid.getLocations().stream().filter(l -> l.getBlockY() == f.getBlockY() + 10)
				.filter(l -> Math.random() <= 0.3)
				.forEach(loc-> ManageHandler.getNMS().setBlock(requireNonNull(XMaterial.EMERALD_BLOCK.parseItem()), loc.getBlock()));
	}
    ...
    private void destroyEmeralds() {
		...
		getArena().getProperties().getCuboid().getLocations().stream().filter(l -> l.getBlockY() == first.getBlockY() + 10)
				.filter(l -> l.getBlock().getType() != Material.AIR).forEach(l -> {
					if (getItem(l.getBlock()).isSimilar(XMaterial.GLASS.parseItem())) {
						...
						ManageHandler.getNMS().playOutParticle(l, 1f, NMS.Particle.CLOUD, 5);
						l.getBlock().setType(Material.AIR);
					}
					else if (Math.random() <= 0.2) ManageHandler.getNMS().setBlock(requireNonNull(XMaterial.GLASS.parseItem()), l.getBlock());
		});
	}
    ...
    public void check() {
		...
		if (getArena().isHardMode() && getTime() % 30 == 0) destroyEmeralds();
    	...
    	playerList.stream().filter(gamePlayer -> gamePlayer.getState() == State.PLAYING_GAME).forEach(gamePlayer -> {
    		...
    		if ((y == param_y  + 2 || y == param_y + 3) && !getArena().getProperties().getCuboid().notInside(player.getLocation())) setVelocityUP(player);
    		else if (y <= param_y) onLose(player, true);
    	});
    }

	private void setVelocityUP(Player player) {
		double yVelocity = player.getVelocity().getY();
		if (yVelocity < 0) player.setVelocity(new Vector(0, Math.abs(yVelocity), 0));
	...
	public void end() {
		...
		getArena().getPlayers().forEach(gamePlayer -> {
			...
			if (player.getLocation().getBlockY() == blockY + 11
					|| player.getLocation().getBlockY() == blockY + 12) onWin(player, false);
	...
	public ItemStack getGameItemStack() {
    	return ItemBuilder.start(requireNonNull(XMaterial.EMERALD_BLOCK.parseItem())).setDisplayName("&2&lJUMP & STAY").build();
	}
