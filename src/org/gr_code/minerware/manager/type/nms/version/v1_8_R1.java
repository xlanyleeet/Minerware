...
public class v1_8_R1 implements NMS {
    ...
    @Override
    public boolean equalsItemStack(ItemStack itemStack, ItemStack itemStack_1) {
        if(!itemStack.hasItemMeta())
            return false;
        ItemMeta itemMeta = itemStack.getItemMeta();
        assert itemMeta != null;
        return Objects.requireNonNull(itemMeta).getDisplayName().equals(Objects.requireNonNull(itemStack_1.getItemMeta()).getDisplayName());
    }
    ...
    public void sendTitle(Player player, String string) {
        ...
        IChatBaseComponent titleJSON = ChatSerializer.a("{'text': '" + string + "'}");
        PacketPlayOutTitle titlePacket = new PacketPlayOutTitle(EnumTitleAction.TITLE, titleJSON, 5, 65, 15);
        connection.sendPacket(titlePacket);
    }

    @Override
    public ItemStack[] getInventoryContents(Player player) {
        int lengthInventory = 40;
        ItemStack[] itemStacks = new ItemStack[lengthInventory];
        for (int i = 0; i < 36; i++) {
        ...
        for(int i = 0; i < 4; i++){
            ItemStack a = player.getInventory().getArmorContents()[i];
            if(a == null)
                continue;
    ...
    public void restoreInventory(Player player, ItemStack[] contents) {
        for (int i = 0; i < 36; i++) {
            ...
            player.getInventory().setItem(i, a);
        }
        player.getInventory().setArmorContents(new ItemStack[]{contents[36], contents[37], contents[38], contents[39]});
    }
    ...
    public void setHurtEntities(FallingBlock block) {
        ((CraftFallingSand)block).getHandle().a(true);
    }
    ...
    public void sendTitle(Player player, String title, String subTitle, int fadeIn, int showTime, int fadeOut) {
        ...
        PacketPlayOutTitle packetPlayOutTitle = new PacketPlayOutTitle(EnumTitleAction.TITLE, titleComponent, fadeIn, showTime, fadeOut);
        PacketPlayOutTitle packetPlayOutSubTitle = new PacketPlayOutTitle(EnumTitleAction.SUBTITLE, subTitleComponent, fadeIn, showTime, fadeOut);
        PacketPlayOutTitle packetPlayOutTimings = new PacketPlayOutTitle(EnumTitleAction.TIMES, titleComponent, fadeIn, showTime, fadeOut);
    ...
    public void updateFireWinEffect(Player player) {
        ...
        for(BlockPosition block : blockPositions) {
            ...
            PlayerConnection playerConnection = ((CraftPlayer)player).getHandle().playerConnection;
            if(player.getWorld().getBlockAt(block.getX(), block.getY()-1, block.getZ()).getType() != Material.AIR)
                playerConnection.sendPacket(packetPlayOutBlockChange);
        }
