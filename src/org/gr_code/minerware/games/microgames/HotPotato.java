...
public class HotPotato extends MicroGame {
	...
	private void chooseHot() {
		...
		for (int i = 0; i < count; i ++) {
			...
			for (int j = 0; j < 9; j ++) player.getInventory().setItem(j, XMaterial.POTATO.parseItem());
    ...
    public void check() {
		...
		if (fires.containsValue(getTime()))
			for (Block bl : fires.keySet()) {
				if (fires.get(bl) != getTime()) continue;
		...
		playerList.stream().filter(gamePlayer -> gamePlayer.getState() == State.PLAYING_GAME).forEach(gamePlayer -> {
    		...
    		if (gamePlayer.getTask() != null && gamePlayer.getTask().equals("hot")) hotIsRunning(player);
    	});
    }
    ...
    private void hotIsRunning(Player player) {
		...
		if (l.clone().add(0, -0.5, 0).getBlock().getType() != Material.AIR
			...
			ManageHandler.getNMS().setBlock(requireNonNull(XMaterial.FIRE.parseItem()), l.getBlock());
			fires.put(l.getBlock(), getTime() - 20);
		}
		player.addPotionEffect(PotionEffectType.SPEED.createEffect(60, 1 + count));
		ManageHandler.getNMS().playOutParticle(l, 1F, NMS.Particle.LAVA, 1F, 20);
		ManageHandler.getNMS().playOutParticle(l, 1F, NMS.Particle.FLAME, 1F, 20);
		if (!ManageHandler.getNMS().oldVersion()) player.setFireTicks(40);
	}
	...
	public void event(Event event) {
		...
		if (requireNonNull(x).getState() != State.PLAYING_GAME) return;
		if (requireNonNull(x2).getState() != State.PLAYING_GAME) return;
		if (!(x.getTask() != null && x2.getTask() == null && x.getTask().equals("hot"))) return;
		HotPotato hotP = (HotPotato) getArena().getMicroGame();
		x2.setTask("hot");x.setTask(null);
		...
		if (secondAch - firstAch > 0) requireNonNull(hotP).achievement.put((secondAch - firstAch) + "", x);
		x.setAchievement(null);
		x2.setAchievement(achForEnt);
		for (int j = 0; j < 9; j ++) p2.getInventory().setItem(j, XMaterial.POTATO.parseItem());
