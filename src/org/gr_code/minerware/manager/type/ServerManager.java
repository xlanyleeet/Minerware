...
public class ServerManager {
    ...
    public static Arena getArena(UUID uuid) {
        return MinerPlugin.getARENA_REGISTRY().stream().filter(player -> player.getPlayer(uuid) != null).findAny().orElse(null);
    }

    @Nullable
    public static Arena getArena(String paramString) {
        return MinerPlugin.getARENA_REGISTRY().stream().filter(player -> player.getProperties().getName().equals(paramString)).findAny().orElse(null);
    }

    @Nullable
    public static Arena getRandomArena(UUID uuid) {
        List<Arena> arenas = MinerPlugin.getARENA_REGISTRY().stream()
                .filter(arena -> (arena.getStage().equals(Arena.Stage.WAITING) || arena.getStage().equals(Arena.Stage.STARTING)))
                .filter(arena -> arena.canJoin(uuid)).sorted(Comparator.comparingInt(Arena::getRequiredPlayers)).collect(Collectors.toList());
        if (arenas.isEmpty())
            return null;
    ...
    public static void startBungeeMode(){
        ...
        System.out.println("[MinerWare] Bungee mode has been enabled. Detected "+MinerPlugin.getARENA_REGISTRY().size() + " arena" +
