...
public class SpiderMan extends MicroGame {
    ...
    private void generateBlocks() {
        ...
        getArena().getProperties().getCuboid().getLocations().stream().filter(l -> l.getBlockY() != first.getBlockY() && l.getBlockY() <= second.getBlockY() - 2)
            ...
            else if (random <= 0.98) ManageHandler.getNMS().setBlock(requireNonNull(XMaterial.SLIME_BLOCK.parseItem()), l.getBlock());
    ...
    public void secondStartGame() {
        rightState = ManageHandler.getNMS().isLegacy() ? PlayerFishEvent.State.FAILED_ATTEMPT : PlayerFishEvent.State.REEL_IN;
        double currentTime = ((double)System.currentTimeMillis()) / 1000;
        ...
        generateBlocks();
        ItemStack item = ItemBuilder.start(Objects.requireNonNull(XMaterial.FISHING_ROD.parseItem())).setUnbreakable(true).build();
    ...
    public void check() {
        ...
        playerList.stream().filter(gamePlayer -> gamePlayer.getState() == State.PLAYING_GAME).forEach(gamePlayer -> {
            Player player = gamePlayer.getPlayer();
                ...
                if (y > first.getBlockY() + 3 && getItem(player.getLocation().add(0,-1,0).getBlock()).isSimilar(gold)) onWin(player, true);
    ...
    public void event(Event event) {
        ...
        try { playerFish(event); } catch (Exception ignored) { }
    }

    private void playerFish(Event event) throws NoSuchFieldException, IllegalAccessException, InvocationTargetException, NoSuchMethodException {
        PlayerFishEvent e = (PlayerFishEvent) event;
        Player p = e.getPlayer();
        if (requireNonNull(getArena().getPlayer(p.getUniqueId())).getState() != State.PLAYING_GAME) return;
        ...
        if (e.getState() == PlayerFishEvent.State.FISHING) setVelocity.invoke(object, p.getLocation().getDirection().normalize().multiply(2));
        if (!(e.getState() == PlayerFishEvent.State.IN_GROUND || e.getState() == rightState)) return;
        if (getArena().getProperties().getCuboid().notInside(location)) return;
        World world = location.getWorld();
        Vector start = location.toVector();
        Vector direction = new Vector(location.getX() - p.getLocation().getX(), location.getY() - p.getLocation().getY(), location.getZ() - p.getLocation().getZ());
        ...
        if (hitBlock == null || requireNonNull(hitBlock).getType() == Material.AIR) return;
        ManageHandler.getNMS().playOutParticle(p.getLocation().add(0,1,0), 0.5F, NMS.Particle.CLOUD, 0F, 35);
