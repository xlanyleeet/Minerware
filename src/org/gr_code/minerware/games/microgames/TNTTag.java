...
public class TNTTag extends MicroGame {
    ...
    public void onLose(Player player, boolean teleport) {
        ...
        ManageHandler.getNMS().playOutParticle(player.getLocation(), 2F, NMS.Particle.LARGE_EXPLOSION, 1F, 10);
        player.getWorld().playSound(player.getLocation(), requireNonNull(XSound.ENTITY_GENERIC_EXPLODE.parseSound()), 5, 1);
    ...
    private List<Location> generateLocations() {
        ...
        return getArena().getProperties().getCuboid().getLocations().stream()
                ...
                .filter(l -> l.distance(getArena().getProperties().getCuboid().getCenter().clone().add(0, 3, 0)) <= Cuboid.getSize(getArena())/2)
    ...
    public void secondStartGame() {
        ...
        for (int i = 0; i < count; i ++) {
            ...
            for (int j = 0; j < 9; j ++) p.getInventory().setItem(j, XMaterial.TNT.parseItem());
    ...
    private void thirdStartGame() {
        ...
        getArena().getPlayers().forEach(x -> {
            Player player = x.getPlayer();
            if (x.getTask() != null && x.getTask().equals("hot")) onLose(player, true);
            ManageHandler.getNMS().sendActionBar(player, bar);
        });
        List<GamePlayer> players = getArena().getPlayers().stream().filter(x->x.getState() == State.PLAYING_GAME).collect(Collectors.toList());
        if (players.size() == 1) {
            setTime(59);
            return;
        }
    ...
    private void tick(int t) {
        String[] cd = {"", "&4&l|", "&4&l||", "&4&l|||", "&4&l||||", "&4&l|||||", "&4&l||||||", "&4&l|||||||", "&4&l||||||||", "&4&l|||||||||", "&4&l||||||||||"};
        String bar = translate(getString("action-bar.timer"));
        getArena().getPlayers().forEach(x->{
            Player p = x.getPlayer();
            ManageHandler.getNMS().sendActionBar(p, translate(cd[t] + bar + cd[t]));
        });
    }
    ...
    public void check() {
        ...
        switch (getTime()) {
            case 500:
            case 300:
                thirdStartGame(); break;
            case 100:
                fifthStartGame(); break;
        }
        ...
        if (getTime() % 20 == 0) {
            if (getTime() >= 500) tick((getTime() - 500) / 20);
            else if (getTime() >= 300) tick((getTime() - 300) / 20);
            else if (getTime() >= 100) tick((getTime() - 100) / 20);
        }
        playerList.stream().filter(gamePlayer -> gamePlayer.getState() == State.PLAYING_GAME).forEach(gamePlayer -> {
            ...
            if (gamePlayer.getTask() != null && gamePlayer.getTask().equals("hot")) {
                ManageHandler.getNMS().playOutParticle(player.getLocation(), 1F, NMS.Particle.LAVA, 1F, 10);
    ...
    public void event(Event event) {
        ...
        if (x2.getState() != State.PLAYING_GAME) return;
        if (!(x.getTask() != null && x2.getTask() == null && x.getTask().equals("hot"))) return;
        x2.setTask("hot");
        x.setTask(null);
        ...
        if (secondAch - firstAch > 0) achievement.put((secondAch - firstAch) + "", x);
        x.setAchievement(null);
        x2.setAchievement(achForEnt);
        for (int j = 0; j < 9; j ++) p2.getInventory().setItem(j, XMaterial.TNT.parseItem());
