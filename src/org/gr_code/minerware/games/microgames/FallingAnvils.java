...
public class FallingAnvils extends MicroGame {
	...
	private void createFloor() {
		...
		getArena().getProperties().getCuboid().getLocations().stream().filter(l -> l.getBlockY() == first.getBlockY())
				.forEach(l -> ManageHandler.getNMS().setBlock(requireNonNull(XMaterial.BEDROCK.parseItem()), l.getBlock()));
	}
	...
	public void secondStartGame() {
		...
		predicateForEach = ManageHandler.getNMS().oldVersion() ? l -> {
			FallingBlock block = requireNonNull(l.getWorld()).spawnFallingBlock(l, Material.ANVIL, (byte) 0);
			ManageHandler.getNMS().setHurtEntities(block);
			countAnvils++;
		} : l -> {
			FallingBlock block = requireNonNull(l.getWorld()).spawnFallingBlock(l, requireNonNull(requireNonNull(XMaterial.ANVIL.parseItem()).getData()));
	...
	public ItemStack getGameItemStack() {
    	return ItemBuilder.start(requireNonNull(XMaterial.ANVIL.parseItem())).setDisplayName("&8&lFALLING ANVILS").build();
	}
