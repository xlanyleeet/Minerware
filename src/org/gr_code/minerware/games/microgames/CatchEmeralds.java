...
public class CatchEmeralds extends MicroGame {
    ...
    private void updateForEachInCheck(GamePlayer gamePlayer, String plus) {
		...
		Stream<ArmorStand> streamArmor = standList.stream().filter(ar -> ar.getLocation().distance(player.getLocation()) <= 1.5);
		for (ArmorStand ar : streamArmor.collect(Collectors.toList()))
			collectEmerald(ar, player, gamePlayer, plus);
		if ((playerY == loseY + 1 || playerY == loseY + 2) && !getArena().getProperties().getCuboid().notInside(player.getLocation())) setVelocityUP(player);
		else if (playerY <= loseY - 1) onLose(player, true);
	}
    ...
    private void collectEmerald(ArmorStand ar, Player player, GamePlayer gamePlayer, String plus) {
		ManageHandler.getNMS().playOutParticle(ar.getLocation(), player, 0.5F, NMS.Particle.HAPPY_VILLAGER, 0, 20);
		player.playSound(player.getLocation(), requireNonNull(XSound.ENTITY_ARROW_HIT_PLAYER.parseSound()), 5, 1);
		gamePlayer.setTask(Integer.toString(Integer.parseInt(gamePlayer.getTask()) + 1));
		if (Integer.parseInt(gamePlayer.getTask()) == 4) onWin(player, false);
		else Utils.sendMessage(player, translate(plus.replace("<emeralds>", gamePlayer.getTask())));
    ...
    private void spawnStand() {
    	if (standList.size() >= (getArena().getCurrentPlayers() + 1) * 2) return;
    	Location location = getRandomLocation(getArena()).add(0, 10, 0);
    	location.setYaw(yaw);
    	ArmorStand ar = (ArmorStand) requireNonNull(location.getWorld()).spawnEntity(location, EntityType.ARMOR_STAND);
    	ar.setGravity(false);
	...
	public ItemStack getGameItemStack() {
    	return ItemBuilder.start(requireNonNull(XMaterial.EMERALD.parseItem())).setDisplayName("&a&lCATCH EMERALDS").build();
	}
