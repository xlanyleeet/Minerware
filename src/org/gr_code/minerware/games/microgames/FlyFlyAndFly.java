...
public class FlyFlyAndFly extends MicroGame {

    private double firstTime;
    public HashMap<String, GamePlayer> achievement;

    public FlyFlyAndFly(Arena arena) {
        super(480, arena, "fly-fly-and-fly");
    ...
    public String getAchievementForMsg() {
        ...
        return achievementMsg.replace("<name>", name).replace("<seconds>", getSeconds(maximum));
    }
    ...
    public void secondStartGame() {
        ...
        getArena().getPlayers().forEach(gamePlayer -> {
            ...
            player.getInventory().setItem(new Random().nextInt(9), XMaterial.APPLE.parseItem());
            player.getInventory().setItem(new Random().nextInt(9), XMaterial.EMERALD.parseItem());
        });
    }
    ...
    public void check() {
        ...
        playerList.stream().filter(gamePlayer -> gamePlayer.getState() == State.PLAYING_GAME).forEach(x->{
            ...
            if (y > first.getBlockY() + 3 && IS.isSimilar(XMaterial.GOLD_BLOCK.parseItem())) onWin(player, true);
    ...
    public ItemStack getGameItemStack() {
        return ItemBuilder.start(requireNonNull(XMaterial.APPLE.parseItem())).setDisplayName("&f&lFLY FLY AND FLY").build();
    }
    ...
    public void event(Event event) {
        ...
        if (e.getItem().isSimilar(XMaterial.APPLE.parseItem())) {
            player.teleport(getArena().getProperties().getLobbyLocationLoser().clone().add(0,1,0));
            onLose(player, true);
            return;
        }
        if (!e.getItem().isSimilar(XMaterial.EMERALD.parseItem())) return;
        float count = getArena().isHardMode() ? 0.8f : 1f;
        player.setVelocity(new Vector(0, count, 0));
